...
def test_uninstall_all(mock_check_call, mock_check_output):
    ...
    mock_check_call.assert_called_with(
        [sys.executable, "-m", "pip", "uninstall", "-y", "package1==1.0", "package2==2.0"]
    )
...
def test_install_package(mock_check_call):
    ...
    mock_check_call.assert_called_with(
        [sys.executable, "-m", "pip", "install", "dummy.tar.gz"]
    )
...
def test_main_single_package(monkeypatch):
    ...
    with mock.patch("install_app.uninstall_all"), \
         mock.patch("install_app.reinstall_requirements"), \
         mock.patch("install_app.install_package") as mock_install:
        install_app.main()
...
def test_main_directory(monkeypatch, tmp_path):
    ...
    with mock.patch("install_app.uninstall_all"), \
         mock.patch("install_app.reinstall_requirements"), \
         mock.patch("install_app.install_from_folder") as mock_folder:
        install_app.main()
